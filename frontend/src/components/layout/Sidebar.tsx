...
function NavItems({ items, pathname, onClose }: { items: typeof navItems; pathname: string; onClose?: () => void }) {
  return (
    <div className="p-2">
      {items.map((item) => {
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-white border-b border-gray-200 h-14 flex items-center px-4 gap-3">
        ...
        >
          <svg className="w-5 h-5 text-gray-600" fill="currentColor" viewBox="0 0 18 14">
        ...
        {logoIcon}
        <span className="font-semibold text-gray-900 text-sm">Settlement OS</span>
      </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          {/* Drawer */}
          <aside className="relative w-72 max-w-[85vw] bg-white h-full flex flex-col shadow-2xl overflow-y-auto">
            ...
            <nav className="flex-1 border-b border-gray-100">
              <NavItems items={visibleItems} pathname={pathname} onClose={() => setMobileOpen(false)} />
            </nav>
            {userFooter}
          </aside>
        </div>
